...
PageID addTupleToBucket(Reln r, Tuple t, PageID p);

PageID getBucketID(Reln r, Bits h)
{
	Bits p;
	if (r->depth == 0)
		p = 1;
...
Bool needToSplit(Reln r)
{
	Count nt = ntuples(r);
	Bool isCa = (nt % (1024 / (10 * nattrs(r)))) == 0;
	return (nt > 0) && isCa;
}
Bool linearSplit(Reln r);
PageID addToRelation(Reln r, Tuple t)
{
	...
	PageID ret = addTupleToBucket(r, t, p);
	if (needToSplit(r))
	{
		linearSplit(r);
	}
...
Bool linearSplit(Reln r)
{
	PageID oldsp = splitp(r);
	r->sp += 1; //change d
	Page pg = getPage(r->data, oldsp);
	addPage(r->data);
	r->npages += 1;
	Bool is_ovflowPage = FALSE;
	PageID toOverride = oldsp;
	while (TRUE)
	{
		Page newpg = newPage();
		char *tuple = pageData(pg);
		for (unsigned int i = 0u; i < pageNTuples(pg); i++)
		{
			Bits h = tupleHash(r, tuple);
			Bits bkID = getBucketID(r, h);
			if (bkID < splitp(r))
			{
				addToPage(newpg, tuple);
			}
			else
			{
				addTupleToBucket(r, tuple, bkID);
				r->ntups--;
			}
			tuple += tupLength(tuple) + 1;
		}
		if (is_ovflowPage == FALSE)
		{
			putPage(r->data, toOverride, newpg);
			is_ovflowPage = TRUE;
		}
		else
		{
			putPage(r->ovflow, toOverride, newpg);
		}
		toOverride = pageOvflow(pg);
		if (toOverride == NO_PAGE)
		{
			if ((r->sp) == (1 << (r->depth)))
			{
				r->depth += 1;
		...
		pg = getPage(r->ovflow, toOverride);
		// free malloc space ??
	}
}
