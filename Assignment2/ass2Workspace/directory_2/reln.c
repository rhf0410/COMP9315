...
{
	int num_tup = r->ntups + 1;
	int split_or_not = num_tup % (1024/(10*r->nattrs));
	if (split_or_not == 0){
		splitReln(r);
	}
	//printf("%s\n", t);
...
// 	addPage(r->data);
// 	r->npages++;
// 	//Tuple *original = malloc(1024*sizeof(Tuple));
//
// 	PageID pid = r->sp;
// 	FILE *data = r->data;
// 	//int index = 0;
...
//
// 	while(pid != NO_PAGE){
// 		Page pg = getPage(data, pid);
// 		free(pg);
// 		if(pageNTuples(pg) == 0){
// 			break;
// 		}
//
// 		Tuple tuple = nextTuple(data, pid, curtup);
// 		Bits hash = tupleHash(r, tuple);
// 		Bits newId = getLower(hash, r->depth + 1);
// 		if(newId == pid){
// 			original[index++] = copyString(tuple);
// 			//printf("%s\n", original[index-1]);
// 		}else{
// 			PageID new_page_id = reScheduleRelation(r, tuple, newId);
// 			if(new_page_id == NO_PAGE){
// 			}
// 		}
// 		curtup += strlen(tuple) + 1;
// 		ntups++;
// 		if(ntups >= pageNTuples(pg)){
// 			Page cover = newPage();
// 			putPage(data, pid, cover);
// 			pid = pageOvflow(pg);
// 			curtup = 0;
// 			ntups = 0;
// 			data = r->ovflow;
...
// 	int i;
// 	for(i=0;i<index;i++){
// 		Tuple tmp = original[i];
// 		PageID rog_page_id = reScheduleRelation(r, tmp, r->sp);
// 		if(rog_page_id == NO_PAGE){
// 			printf("Failed\n");
// 		}
// 		free(tmp);
// 	}
...
void splitReln(Reln r){
	...
	Tuple *new_tup = malloc(1024 * sizeof(Tuple));
	PageID pid = r->sp;
	FILE * data = r->data;
	int index = 0;
	Offset cur_tup = 0;
	...
	while (pid != NO_PAGE){
		Page page = getPage(data, pid);

		if(pageNTuples(page) == 0){
			break;
		}

		Tuple tmp = nextTuple(data, pid, cur_tup);
		Bits hash;
		Bits newid;
		...
		if (newid == pid){
			new_tup[index++] = copyString(tmp);
		} else {
			//insert tuple in new page
			if(tmp[0] == '1' && tmp[1] == '0' && tmp[2] == '8' && tmp[3] == '0')
				printf("Tmp: %s, new id: %d\n",tmp, newid);
			printf("Tmp: %s, new id: %d\n",tmp, newid);
			PageID new_page = reScheduleRelation(r, tmp, newid);
			if (new_page == NO_PAGE)
				printf("No Page\n");
		}
		cur_tup += strlen(tmp)+1;
		num_tups++;

		if (num_tups >= pageNTuples(page)){
			Page cover = newPage();
			putPage(data, pid, cover);
			pid = pageOvflow(page);
			num_tups = 0;
	...
	for (i = 0; i < index; i++){
		Tuple tmp = new_tup[i];
		if (reScheduleRelation(r, tmp, r->sp) == NO_PAGE)
...
//Re-schedule tuple to relation.
PageID reScheduleRelation(Reln r, Tuple t, PageID pid){
